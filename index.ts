...
export function format(
  ...
  function print_selectorlist(node: SelectorList) {
    ...
    node.children.forEach((selector, item) => {
      ...
      let end =
        item.next !== null ? start_offset(item.next.data) : end_offset(node);
  ...
  function print_simple_selector(
    node: Selector | PseudoClassSelector | PseudoElementSelector
  ) {
    let buffer = EMPTY_STRING;
      ...
      switch (child.type) {
        ...
        case TYPE_PSEUDO_ELEMENT_SELECTOR: {
          ...
          if (
            pseudo === "before" ||
            pseudo === "after" ||
            child.type === TYPE_PSEUDO_ELEMENT_SELECTOR
          ) {
            buffer += COLON;
          }
        ...
        case "Nth": {
          ...
          if (child.selector !== null) {
            ...
            buffer +=
              SPACE + "of" + SPACE + print_simple_selector(child.selector);
          }
          break;
        }
  ...
  function print_block(node: Block) {
    ...
    let opening_comment = print_comment(
      start_offset(node),
      start_offset(children.first!)
    );
    if (opening_comment) {
    ...
    children.forEach((child, item) => {
      if (item.prev !== null) {
        let comment = print_comment(
          end_offset(item.prev.data),
          start_offset(child)
        );
        if (comment) {
          buffer += indent(indent_level) + comment + NEWLINE;
    ...
    let closing_comment = print_comment(
      end_offset(children.last!),
      end_offset(node)
    );
    if (closing_comment) {
  ...
  function print_prelude(node: AtrulePrelude | Raw) {
    ...
    return buffer
      .replace(
        /\s*([:,])/g,
        buffer.toLowerCase().includes("selector(") ? "$1" : "$1 "
      ) // force whitespace after colon or comma, except inside `selector()`
      ...
      .replace(
        /([^<>=\s])([<>])([^<>=\s])/g,
        `$1${OPTIONAL_SPACE}$2${OPTIONAL_SPACE}$3`
      ) // add spacing around < or > except when it's part of <=, >=, =>
      ...
      .replace(
        ...
        (_, left, operator, right) => {
          ...
          let space =
            operator === "+" || operator === "-" ? SPACE : OPTIONAL_SPACE;
  ...
  function print_list(children: List<CssNode>) {
    ...
    children.forEach((node, item) => {
      if (node.type === "Identifier") {
        ...
        buffer +=
          OPEN_PARENTHESES + print_list(node.children) + CLOSE_PARENTHESES;
